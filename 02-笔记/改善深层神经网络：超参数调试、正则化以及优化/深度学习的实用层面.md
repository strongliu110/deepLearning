...
$\sigma ^{2}=\frac{1}{m}\sum_{i=1}^{m}{\left( X^{\left( i \right)} \right)^{2}}$
...
这里忽略了常数项b。**为了让z不会过大或者过小，思路是让w与n有关，且n越大，w应该越小才好，这样能够保证z不会过大**。一种方法是在初始化权值w时，令其方差等于一个较小的值。
...
反向传播（Back Propagation）神经网络有一项重要的测试是**梯度检查（gradient checking）**，目的是检查验证反向传播过程中梯度下降算法是否正确。通常的做法是, 利用极限原理, 设$g\left( \theta  \right)$是$f\left( \theta  \right)$的导数, 则梯度检查就是验证$\frac{f\left( \theta +\epsilon  \right)-f\left( \theta  \right)}{\epsilon }$近似$g\left( \theta  \right)$, 或者用$\frac{f\left( \theta +\epsilon  \right)-f\left( \theta -\epsilon  \right)}{2\epsilon }$,能获得更高的精度函数。
...
计算完所有$\theta _{i}$的近似梯度后，可以计算$d\theta _{approx}$与$d\theta$的欧氏（Euclidean）距离来比较二者的相似度。公式如下：
